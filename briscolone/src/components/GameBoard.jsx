...
export function GameBoard({ gameState, localPlayerId, updateGame }) {
  ...
  const renderCardImage = (code) => {
    if (!code) return null;
    return <img src={`/carte/${code}.png`} alt={code} style={{ width: '90px', height: '140px', borderRadius: '8px', boxShadow: '0 4px 10px rgba(0,0,0,0.4)', display: 'block' }} />;
  };
  ...
  if (gameState?.fase === 'risultato_finale' || partitaFinita) {
    ...
    return (
      <div style={{ color: 'white', background: '#1e293b', padding: '40px', borderRadius: '15px', maxWidth: '600px', margin: '30px auto', textAlign: 'center' }}>
        ...
        <button onClick={resetMassivoNuovaPartita} style={{ padding: '15px 35px', fontSize: '18px', background: '#10b981', color: 'white', fontWeight: 'bold', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>
  ...
  const mostraSemeRealmente = manoAttuale > 1; 
    ...
    <div className="game-board" style={{ color: 'white', padding: '15px' }}>
      <div className="game-info" style={{ textAlign: 'center', marginBottom: '20px', background: '#1e293b', padding: '15px', borderRadius: '10px' }}>
        ...
        <h3 style={{ margin: '0 0 5px 0' }}>
          💥 Seme di Briscola:{' '}
          <span style={{ color: mostraSemeRealmente ? '#10b981' : '#ef4444' }}>
      ...
      <div className="table-center" style={{ background: '#064e3b', padding: '25px', borderRadius: '15px', minHeight: '260px' }}>
        <h3 style={{ textAlign: 'center', margin: '0 0 15px 0', color: '#a7f3d0' }}>Carte sul banco</h3>
